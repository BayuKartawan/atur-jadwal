  ...
  <AppCard bodyClass="!p-4 lg:!p-6">
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 leading-none">Manajemen Kelas</h3>
          <p class="text-xs font-medium text-slate-500 mt-1">Kelola rombel siswa</p>
        </div>
      </div>
    </template>
    ...
    <div class="flex flex-col gap-6">
      <div class="flex gap-3">
        <AppInput 
          ...
          class="flex-1"
        />
        <AppButton @click="handleAdd" size="icon" class="w-12 h-12 !p-0 !bg-sky-500 hover:bg-sky-600 shadow-sky-100 dark:shadow-none">
          <Plus :size="20" />
        </AppButton>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-4">
        ...
        <div v-if="classes.length === 0" class="col-span-full py-12 text-center opacity-40">
          <Layers :size="32" class="mx-auto mb-2 text-slate-300 dark:text-slate-700" />
          <p class="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Belum ada data kelas</p>
        </div>
      </div>
    </div>
  </AppCard>
