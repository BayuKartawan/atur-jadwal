        ...
        >
          <div v-for="t in teachers" :key="t.id" 
            ...
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-bold text-xs uppercase group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                {{ t.name.charAt(0) }}
              </div>
              <span class="font-bold text-slate-700 dark:text-slate-300 transition-colors group-hover:text-slate-900 dark:group-hover:text-white">
                {{ t.name }}
                <AppBadge v-if="getTeacherHomeroomClass(t.id)" variant="success" class="ml-2">
                  Wali {{ getTeacherHomeroomClass(t.id) }}
                </AppBadge>
              </span>
            </div>
            <AppButton @click="$emit('remove', t.id)" variant="danger" size="icon" class="lg:opacity-0 group-hover:opacity-100 !p-2 !rounded-xl">
              <Trash2 :size="16" />
            </AppButton>
          </div>
        </TransitionGroup>
