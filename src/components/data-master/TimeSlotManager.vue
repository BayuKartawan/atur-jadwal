...
import { Plus, Trash2, Clock, Edit2, Save, X, GripVertical } from 'lucide-vue-next';
...
const emit = defineEmits(['addPeriod', 'removePeriod', 'updatePeriod', 'updatePeriodOrder']);
...
<template>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <AppCard overflowVisible>
            <div class="flex flex-col gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">Tambah Jam Pelajaran</h3>
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex gap-2">
                        <input v-model="newPeriod" type="text" placeholder="Contoh: 07:15 - 07:45"
                        ...
                        <AppButton @click="handleAdd" :disabled="!newPeriod.trim()" variant="primary" size="icon"
                            class="!w-11 !h-11 !rounded-xl shrink-0 !bg-orange-600 hover:!bg-orange-700 shadow-lg shadow-orange-500/20">
                            <Plus :size="20" />
                        </AppButton>
                    </div>

                    <label class="flex items-center gap-2 self-start cursor-pointer group">
                        <input type="checkbox" v-model="isNewPeriodBreak" class="checkbox-input hidden" />
                        <div class="w-4 h-4 rounded border border-slate-300 dark:border-slate-600 flex items-center justify-center transition-colors"
                            :class="isNewPeriodBreak ? 'bg-orange-500 border-orange-500' : 'bg-transparent'">
                            <div v-if="isNewPeriodBreak" class="w-2 h-2 bg-white rounded-sm"></div>
                        </div>
                        <span
                            class="text-xs font-bold text-slate-500 group-hover:text-orange-600 transition-colors select-none">Set
                            sebagai Jam Istirahat</span>
                    </label>
                </div>
            </div>
        </AppCard>
        ...
        <div class="flex flex-col gap-3">
            <h3 class="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 ml-1">Daftar Jam
            </h3>
            ...
            <TransitionGroup enter-active-class="transition-all duration-300 ease-out"
                ...
                <div v-for="(period, index) in periodTimes" :key="index" draggable="true"
                    @dragstart="onDragStart(index)" @dragover="onDragOver" @drop="onDrop(index)" @dragend="onDragEnd"
                    :class="['group bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between gap-3 hover:shadow-md transition-all cursor-move', draggedIndex === index ? 'opacity-50 ring-2 ring-indigo-500/20' : '']">

                    <div class="flex items-center gap-4 flex-1">
                        <div class="text-slate-300 cursor-grab active:cursor-grabbing hover:text-indigo-400">
                        ...
                        <div v-if="editingIndex === index" class="flex-1 flex flex-col gap-2">
                            <input v-model="editValue" type="text" @keyup.enter="saveEditing(index)"
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" v-model="editIsBreak" class="checkbox-input hidden" />
                                <div class="w-3 h-3 rounded border border-slate-300 dark:border-slate-600 flex items-center justify-center transition-colors"
                                    :class="editIsBreak ? 'bg-orange-500 border-orange-500' : 'bg-transparent'">
                                    <div v-if="editIsBreak" class="w-1.5 h-1.5 bg-white rounded-sm"></div>
                                </div>
                                <span class="text-[10px] font-bold text-slate-400 select-none">Istirahat</span>
                            </label>
                        </div>
                        <div v-else class="flex items-center gap-3">
                            <span class="text-sm font-bold text-slate-700 dark:text-slate-200 font-mono tracking-wide">
                                {{ typeof period === 'object' ? period.time : period }}
                            </span>
                            <span v-if="(typeof period === 'object' && period.isBreak)"
                    ...
                    <div class="flex items-center gap-1">
                        <template v-if="editingIndex === index">
                            <button @click="saveEditing(index)"
                        ...
                        <template v-else>
                            <button @click="startEditing(index, period)"
                            ...
                            <button @click="$emit('removePeriod', index)"
            ...
            <div v-if="periodTimes.length === 0" class="py-8 text-center opacity-50">
                <p class="text-xs italic text-slate-500">Belum ada jam pelajaran diatur</p>
            </div>
        </div>
    </div>
</template>
