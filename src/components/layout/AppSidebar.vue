  ...
  >
    ...
    <div class="p-4 border-t border-white/5 space-y-2">
      <router-link to="/about" @click="handleNavClick" v-slot="{ isActive }">
        <button :class="['w-full flex items-center gap-4 px-4 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all', isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-white/40 hover:text-white hover:bg-white/5']">
          <Info :size="18" />
          Tentang Aplikaksi
        </button>
      </router-link>
