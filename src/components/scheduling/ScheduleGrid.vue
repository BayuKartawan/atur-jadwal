...
const gridTemplate = computed(() => `${timeColWidth.value} repeat(${props.classes.length}, ${classColWidth.value})`);
</script>

<template>
  <div class="flex-1 overflow-auto bg-slate-50 dark:bg-slate-950 custom-scrollbar transition-colors duration-300">
    <div class="bg-white dark:bg-slate-900 lg:rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border-b lg:border border-slate-100 dark:border-slate-800 min-w-fit">
      <!-- Grid Header (Classes) -->
      <div class="grid sticky top-0 bg-slate-900 dark:bg-slate-950 text-white font-bold text-[9px] lg:text-[10px] text-center z-30 shadow-md" 
        :style="{ gridTemplateColumns: gridTemplate }">
        <div class="p-3 lg:p-4 border-r border-slate-800 dark:border-slate-800 sticky left-0 bg-slate-900 dark:bg-slate-950 z-40 flex items-center justify-center uppercase tracking-widest text-slate-400 dark:text-slate-500">JAM</div>
        <div v-for="c in classes" :key="c" 
          class="p-3 lg:p-4 border-r border-slate-800 uppercase truncate tracking-wider lg:tracking-[0.2em] font-black">
          KELAS {{ c }}
        </div>
      </div>
      ...
      <div v-for="(day, dIdx) in DAYS" :key="day" class="border-b last:border-b-0">
        <div class="sticky top-[42px] lg:top-[48px] bg-indigo-50/90 dark:bg-indigo-950/90 backdrop-blur-sm text-indigo-900 dark:text-indigo-300 font-black px-4 lg:px-6 py-2 lg:py-2.5 text-[9px] lg:text-[10px] uppercase tracking-[0.2em] lg:tracking-[0.3em] border-y border-indigo-100/50 dark:border-indigo-900/50 flex items-center gap-2 z-20">
        ...
        <template v-for="(time, pIdx) in PERIOD_TIMES" :key="pIdx">
          <div class="grid border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors" 
            ...
            <!-- Fixed Time Column -->
            <div class="p-2 lg:p-3 border-r border-slate-100 dark:border-slate-800 sticky left-0 bg-white dark:bg-slate-900 flex flex-col items-center justify-center z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
              <span class="text-[8px] lg:text-[9px] font-black text-slate-300 dark:text-slate-600">#{{ pIdx+1 }}</span>
              <span class="text-[9px] lg:text-[10px] font-black text-slate-600 dark:text-slate-400 mt-0.5">{{ time.split(' ')[0] }}</span>
            </div>
            ...
            >
              ...
              <div v-if="isSlotDisabled(dIdx, pIdx, cls)" 
                ...
                <span class="text-[7px] lg:text-[8px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest text-white">OFF</span>
              </div>

              <!-- Content State -->
              <template v-else-if="getSlot(dIdx, pIdx, cls)">
                <div class="w-full flex flex-col h-full justify-center">
                  <div class="text-[9px] lg:text-[10px] font-black text-slate-900 dark:text-slate-100 uppercase leading-tight lg:leading-snug line-clamp-2 px-1 mb-1">
                    {{ getAllocDetails(getSlot(dIdx, pIdx, cls))?.subjectName }}
                  </div>
                  <div class="text-[7px] lg:text-[8px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-tight line-clamp-1 border-t border-slate-100 dark:border-slate-800 pt-1 mt-auto">
              ...
              <!-- Empty State -->
              <div v-else class="text-[7px] lg:text-[8px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0"
                :class="isDisableMode ? 'text-rose-500' : 'text-emerald-500'">
                {{ isDisableMode ? "OFF" : "+ ADD" }}
              </div>
          ...
          <div v-if="pIdx === 3 || pIdx === 5" 
            ...
            <div class="p-2 border-r border-amber-100 dark:border-amber-900 sticky left-0 bg-amber-50 dark:bg-amber-950 flex flex-col items-center justify-center z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
              ...
              <span class="text-[7px] lg:text-[8px] font-black text-amber-500 dark:text-amber-600">{{ pIdx === 3 ? (isMobile ? '09:15' : '09:15-10:00') : (isMobile ? '11:00' : '11:00-11:20') }}</span>
            </div>
            <div class="flex items-center justify-center p-2 text-[9px] lg:text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase lg:tracking-[1em] tracking-widest italic select-none">
