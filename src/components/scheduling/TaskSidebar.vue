...
defineEmits(['update:taskClassFilter', 'update:taskSearchQuery', 'selectAlloc', 'resetSlots', 'closeSidebar']);
...
const allocationsWithRemainingJtm = computed(() => {
  ...
  props.classes.forEach(cls => {
    ...
    const remaining = classAllocations.filter(a => props.getUsedJtm(a.id) < a.jtm).length;
...
<template>
  <div
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar flex flex-col"
      :class="isDisableMode ? 'opacity-20 pointer-events-none' : 'opacity-100 transition-opacity duration-300'">
      <!-- Filter Section -->
      <div class="p-6 space-y-4">
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <button @click="$emit('update:taskClassFilter', 'all')"
              ...
              <span v-if="totalAllocationsWithRemainingJtm > 0"
            ...
            <button v-for="c in classes" :key="c" @click="$emit('update:taskClassFilter', c)"
              ...
              <span v-if="allocationsWithRemainingJtm[c] > 0"
                class="absolute -top-1.5 -right-1.5 min-w-[16px] h-[16px] flex items-center justify-center rounded-full text-[8px] font-black shadow-md px-1"
                :class="taskClassFilter === c
                  ? 'bg-white text-indigo-600'
                  : 'bg-amber-500 text-white'">
                {{ allocationsWithRemainingJtm[c] }}
              </span>
              <!-- Completed indicator -->
              <span v-else class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 shadow-md"></span>
            </button>
          </div>
        </div>
        ...
        <div class="relative group">
          ...
          <button v-if="taskSearchQuery" @click="$emit('update:taskSearchQuery', '')"
      ...
      <div class="px-6 pb-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
        <template v-for="cls in taskSidebarClasses" :key="cls">
          <div
            v-if="allocations.filter(a => a.classId === cls && (!taskSearchQuery || getAllocDetails(a.id)?.subjectName.toLowerCase().includes(taskSearchQuery.toLowerCase()))).length > 0">
            ...
            <div
              ...
              <div
                ...
                {{ cls }}</div>
              <span class="text-[10px] font-black text-white dark:text-slate-100 uppercase tracking-[0.2em]">KELAS {{
                cls }}</span>
            </div>

            <div class="space-y-3">
              <div
                v-for="a in allocations.filter(a => a.classId === cls && (!taskSearchQuery || getAllocDetails(a.id)?.subjectName.toLowerCase().includes(taskSearchQuery.toLowerCase())))"
                ...
                <div @click="getUsedJtm(a.id) < a.jtm && $emit('selectAlloc', a)"
                  ...
                  <div v-if="selectedAllocation?.id === a.id"
                ...
                <button v-if="getUsedJtm(a.id) > 0" @click.stop="$emit('resetSlots', a)"
