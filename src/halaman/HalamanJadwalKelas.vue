...
import AppButton from "../komponen/ui/TombolAplikasi.vue";
import AppSelect from "../komponen/ui/PilihanAplikasi.vue";
...
<template>
  <div class="h-full flex flex-col bg-slate-50/50 dark:bg-slate-950/50">
    <!-- Header -->
    <header
      class="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 border-b border-slate-100 dark:border-slate-800"
    >
      <div
      ...
      >
        <div class="flex items-center gap-3 lg:gap-6">
          <div
            class="w-10 h-10 lg:w-12 lg:h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white shrink-0"
          >
        ...
        <div class="flex items-center gap-2 lg:gap-3 w-full sm:w-auto">
          ...
          <AppButton
            ...
            variant="primary"
            class="!rounded-xl !py-2.5 !h-[42px] lg:!h-auto !px-4 !bg-indigo-600 hover:!bg-indigo-700 disabled:shadow-none shrink-0"
          >
            <template #icon-left>
              <Printer :size="16" />
            </template>
            <span
              class="hidden md:inline text-[10px] font-semibold uppercase tracking-widest"
              >Cetak</span
            >
          </AppButton>
    ...
    <div class="flex-1 overflow-auto custom-scrollbar p-4 lg:p-8">
      ...
      >
        ...
        <p
          class="text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400"
        >
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        <div
          ...
          :key="idx"
          class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col group hover:shadow-md"
        >
          <div
          ...
          >
            <h4
              class="font-semibold text-slate-800 dark:text-slate-200 uppercase tracking-wider text-sm"
            >
              {{ day.day }}
            </h4>
          ...
          <div class="divide-y divide-slate-50 dark:divide-slate-800/50">
            ...
            >
              <div
                class="w-14 shrink-0 py-1.5 flex flex-col items-center justify-center bg-slate-100 dark:bg-slate-800 rounded-xl text-[9px] font-semibold text-slate-500 dark:text-slate-400 tracking-tighter"
              >
              ...
              >
                <div v-if="period.isBreak" class="flex items-center">
                  <span
                    class="px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-widest bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400"
                  >
                    Istirahat
                  </span>
                </div>
                ...
                >
                  ...
                  <span
                    class="text-[9px] font-semibold uppercase tracking-widest text-rose-500"
                    >Nonaktif</span
                  >
                </div>
                <div v-else-if="period.alloc">
                  <div
                    class="font-semibold text-slate-900 dark:text-slate-100 text-[11px] uppercase truncate mb-1"
                ...
                <div v-else class="flex items-center">
                  <span
                    class="text-[9px] font-semibold uppercase tracking-[0.2em] text-slate-300 dark:text-slate-700 italic"
                  >
                    Kosong
                  </span>
                </div>
    ...
    <div
      ...
      class="fixed inset-0 z-[100] bg-slate-900/90 flex flex-col overflow-hidden animate-in fade-in"
    >
      <!-- Toolbar -->
      <div
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <h2 class="text-white font-semibold uppercase text-xs tracking-widest">
        ...
        <div class="flex items-center gap-3">
          <button
            @click="handlePrint"
            class="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2 rounded-xl text-[10px] font-semibold uppercase tracking-widest"
          >
            Cetak Sekarang
          </button>
          <button
      ...
      >
        ...
        <div
          id="print-area"
          class="bg-white text-black p-[2.5cm] origin-top print:shadow-none print:p-0"
          style="width: 210mm; min-height: 330mm"
        >
          <!-- Paper Content -->
          <div class="text-center mb-8 border-b-2 border-black pb-4">
            <h1 class="text-sm font-semibold uppercase mb-1">{{ schoolName }}</h1>
            <h2 class="text-lg font-semibold uppercase tracking-widest mb-1">
              JADWAL PELAJARAN
            </h2>
          ...
          >
            <thead>
              <tr class="bg-slate-100 print:bg-slate-50">
                <th class="border-2 border-black py-3 px-1 w-20 font-semibold">
                  WAKTU
                </th>
                <th
                  v-for="day in DAYS"
                  :key="day"
                  class="border-2 border-black py-3 px-1 font-semibold"
                >
                  {{ day.toUpperCase() }}
                </th>
              </tr>
            ...
            <tbody>
              <tr v-for="(row, rIdx) in tableRows" :key="rIdx">
                <!-- Time Column -->
                <td
                  class="border-2 border-black p-2 text-center font-semibold bg-slate-50/50"
                >
                  {{ row.time.replace(" - ", "\n") }}
                </td>
                ...
                <template v-for="(cell, cIdx) in row.cells" :key="cIdx">
                  <td
                    ...
                    :class="[cIdx === 0 ? '' : 'hidden']"
                    :colspan="DAYS.length"
                    class="border-2 border-black p-2 text-center font-semibold italic bg-amber-50 uppercase tracking-[0.5em]"
                  >
                    ISTIRAHAT
                  </td>
                  <td
                  ...
                  >
                    ...
                    >
                      <div
                        class="font-semibold leading-tight uppercase line-clamp-2"
                      >
