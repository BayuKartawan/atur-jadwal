    ...
    <div class="flex flex-col gap-6">
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <AppButton
          @click="handleAdd"
          size="icon"
          class="w-full sm:w-12 h-12 !p-0 !bg-amber-500 hover:bg-amber-600 dark:shadow-none shrink-0"
        >
          <Plus :size="20" />
        </AppButton>
      </div>
      ...
      <div class="space-y-3">
        ...
        >
          <div
            v-for="s in subjects"
            :key="s.id"
            class="flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl group hover:border-amber-200 dark:hover:border-amber-500 hover:bg-amber-50/30 dark:hover:bg-amber-950/20 relative overflow-visible hover:z-[10] focus-within:z-[20]"
          >
            <div class="flex items-center gap-4 min-w-0 flex-1 mr-4">
              ...
              <div v-else class="min-w-0 flex-1">
                <span
                  class="font-bold text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white block truncate"
                  >{{ s.name }}</span
                >
                <AppBadge
                  :variant="
