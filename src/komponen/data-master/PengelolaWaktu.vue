...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="flex flex-col gap-2">
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none placeholder:text-slate-400"
            />
            <AppButton
              @click="handleAdd"
              ...
              variant="primary"
              size="icon"
              class="!w-11 !h-11 !rounded-xl shrink-0 !bg-orange-600 hover:!bg-orange-700"
            >
              <Plus :size="20" />
            </AppButton>
          </div>
          ...
          >
            ...
            />
            <div
              class="w-4 h-4 rounded border border-slate-300 dark:border-slate-600 flex items-center justify-center"
              :class="
                isNewPeriodBreak
            ...
            <span
              class="text-xs font-bold text-slate-500 group-hover:text-orange-600 select-none"
    ...
    <div class="flex flex-col gap-3">
      <h3
        class="text-sm font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400 ml-1"
      >
        Daftar Jam
      </h3>
      ...
      <TransitionGroup
        ...
        leave-active-class="transition-all ease-in absolute w-full"
      ...
      >
        <div
          ...
          @dragend="onDragEnd"
          :class="[
            'group bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 hover:shadow-md cursor-move',
            draggedIndex === index
        ...
        >
          <div class="flex items-center gap-4 flex-1">
            ...
            <div
              class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 text-xs font-semibold select-none"
            >
              {{ index + 1 }}
            </div>
            ...
            >
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                />
                <div
                  class="w-3 h-3 rounded border border-slate-300 dark:border-slate-600 flex items-center justify-center"
                  :class="
                    editIsBreak
                      ? 'bg-orange-500 border-orange-500'
                      : 'bg-transparent'
            ...
            <div v-else class="flex items-center gap-3">
              ...
              <span
                ...
                class="px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[9px] font-semibold uppercase tracking-wider"
              >
                ISTIRAHAT
              </span>
            </div>
          ...
          <div class="flex items-center gap-1">
            <template v-if="editingIndex === index">
              <button
                ...
                class="w-8 h-8 rounded-lg flex items-center justify-center text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/30"
              >
                <Save :size="16" />
              </button>
              <button
                @click="cancelEditing"
                class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
                <X :size="16" />
              </button>
            </template>
            <template v-else>
              <button
                ...
                class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30"
              >
                <Edit2 :size="16" />
              </button>
              <button
                @click="$emit('removePeriod', index)"
                class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30"
              >
                <Trash2 :size="16" />
              </button>
            </template>
