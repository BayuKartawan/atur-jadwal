...
<template>
  <div
    class="fixed inset-y-0 left-0 w-[85vw] sm:w-80 lg:relative lg:w-80 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col h-full lg:shadow-lg dark:shadow-black/20 z-[60] lg:z-20 transform"
    :class="
  ...
  >
    ...
    >
      <button
        ...
        class="lg:hidden absolute top-4 right-4 p-2 text-slate-400 hover:text-rose-500"
      >
        <X :size="20" />
      </button>
    </div>
    ...
    <div
      v-if="isDisableMode"
      class="m-6 p-4 bg-rose-500 rounded-2xl text-[10px] text-white leading-relaxed"
    >
      <p
        class="font-semibold mb-1 uppercase tracking-widest flex items-center gap-2 italic"
      >
    ...
    >
      ...
      <div class="p-6 space-y-4">
        <div>
          <div
            class="text-[9px] font-semibold text-slate-400 uppercase mb-3 flex items-center gap-2 tracking-widest"
          >
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              class="px-4 py-2 rounded-xl text-[9px] font-semibold uppercase border-2 relative"
              :class="
                taskClassFilter === 'all'
                  ? 'bg-indigo-600 border-indigo-600 text-white dark:shadow-none'
            ...
            >
              ...
              <span
                ...
                class="absolute -top-1.5 -right-1.5 min-w-[16px] h-[16px] flex items-center justify-center rounded-full text-[8px] font-semibold px-1"
                :class="
                  taskClassFilter === 'all'
        ...
        <div class="relative group">
          <Search
            class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-500"
            :size="16"
          />
          <input
            type="text"
            ...
            class="w-full pl-11 pr-10 py-3 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-2xl border-transparent outline-none font-bold text-xs focus:bg-white dark:focus:bg-slate-900 focus:border-emerald-500/20 placeholder:text-slate-300 dark:placeholder:text-slate-600 text-slate-900 dark:text-slate-100"
          />
          <button
            v-if="taskSearchQuery"
            @click="$emit('update:taskSearchQuery', '')"
            class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30"
          >
      ...
      <div class="px-6 pb-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
        <template v-for="cls in taskSidebarClasses" :key="cls">
          ...
          >
            ...
            <div
              class="flex items-center gap-3 mb-4 bg-slate-900 dark:bg-slate-800/80 px-4 py-2.5 rounded-2xl dark:shadow-none border border-slate-800 dark:border-slate-700"
            >
              <div
                class="w-7 h-7 bg-emerald-500 rounded-xl flex items-center justify-center text-white text-[10px] font-semibold italic"
              >
                {{ cls }}
              </div>
              <span
                class="text-[10px] font-semibold text-white dark:text-slate-100 uppercase tracking-[0.2em]"
                >KELAS {{ cls }}</span
              >
            </div>
            ...
            <div class="space-y-3">
              ...
              >
                <div
                  ...
                  class="p-4 rounded-[1.25rem] border-2 text-left cursor-pointer relative overflow-hidden group"
                  :class="[
                ...
                >
                  <div
                    class="text-[11px] font-semibold uppercase text-slate-800 dark:text-slate-100 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 mb-0.5 line-clamp-1 pr-6 tracking-tight"
                  >
                  ...
                  <div class="flex items-center gap-3">
                    <div
                      class="flex-1 bg-slate-100 dark:bg-slate-700/50 h-2.5 rounded-full overflow-hidden border border-slate-200/20 dark:border-slate-700/50"
                    >
                      <div
                        class="bg-gradient-to-r from-emerald-400 to-emerald-600 h-full ease-out shadow-[0_0_8px_rgba(16,185,129,0.4)]"
                        :style="{
                    ...
                    <span
                      class="text-[10px] font-semibold tabular-nums min-w-[32px] text-right"
                      :class="
                        getUsedJtm(a.id) >= a.jtm
                ...
                <button
                  ...
                  class="absolute -top-1.5 -right-1.5 p-2.5 bg-white dark:bg-slate-800 text-orange-500 hover:text-white hover:bg-orange-500 rounded-xl border border-orange-50 dark:border-slate-700 lg:opacity-0 group-hover:opacity-100 hover:scale-110 z-10"
                >
