...
<template>
  <div 
    ...
    ref="dropdownRef"
  >
    <label v-if="label" class="text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-1">
      {{ label }}
    </label>
    ...
    <div 
      ...
      :class="[
        'flex items-center gap-2 bg-slate-50 dark:bg-slate-800 rounded-2xl border cursor-pointer select-none outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50',
    ...
    <!-- Dropdown Menu -->
      <div 
        v-if="isOpen"
        class="absolute z-[9999] top-full left-0 right-0 mt-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden flex flex-col max-h-[320px]"
      >
        <!-- Search -->
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-1">
          <div 
            ...
            :class="[
              'px-4 py-2.5 rounded-xl text-sm font-medium flex items-center justify-between group mb-0.5',
          ...
          <div v-if="filteredOptions.length === 0" class="py-12 text-center opacity-40">
            ...
            <p class="text-[10px] font-semibold uppercase tracking-widest text-slate-500">Tidak ditemukan</p>
          </div>
        </div>
      </div>
